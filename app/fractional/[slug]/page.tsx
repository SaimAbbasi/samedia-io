...
import { fractionalServices, getFractionalService } from '@/lib/fractional-data'

export function generateStaticParams() {
  return fractionalServices.map((s) => ({ slug: s.slug }))
}

export async function generateMetadata({ params }: { params: { slug: string } }) {
  const service = getFractionalService(params.slug)
  if (!service) return {}
  const desc = service.description.slice(0, 160)
  const title = `${service.name} | SA Media Fractional C-Suite`
  return {
    title,
    description: desc,
    keywords: [service.name, 'SA Media', 'Fractional C-Suite', 'fractional executive', 'Toronto', 'New York', 'Dubai', ...service.includes.slice(0, 5)].join(', '),
    openGraph: {
      title,
      description: desc,
      type: 'website',
    ...
    alternates: {
      canonical: `https://samedia.io/fractional/${params.slug}`,
    },
  }
}

export default function FractionalServicePage({ params }: { params: { slug: string } }) {
  const service = getFractionalService(params.slug)
  if (!service) notFound()
  ...
  return (
    <>
      <Script
        id={`fractional-schema-${service.slug}`}
      ...
      <section className="bg-dark pt-28 md:pt-36 pb-16 md:pb-24 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <h1 className="font-heading text-4xl sm:text-5xl md:text-8xl text-white leading-none mb-6">
            {service.name}
          </h1>
          <p className="font-body text-xl text-lime mb-4">
            {service.tagline}
          </p>
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            ...
            <h2 className="font-heading text-4xl md:text-6xl text-dark leading-tight">
              Let&apos;s put {service.name}<br />to work for your business.
            </h2>
          </div>
